...
また、ソースコードは Github (\url{https://github.com/example/webpro_06}) にて公開している。

\section{開発したアプリケーション}
\subsection{IIDX Score Manager (beatmania IIDX スコア管理)}
音楽ゲーム「beatmania IIDX」のスコアを管理するツール。
既存のデータベース版を参考に、メモリ管理版として再実装を行った。
ユーザーはプレイした楽曲のスコア、難易度、クリアランプ等を記録できる。

\subsection{Course Manager (履修科目管理)}
大学の授業科目を管理するツール。
科目名、担当教員、開講期（前期・後期など）、時限、単位数、区分（必修・選択など）を登録し、
履修計画の確認や単位数の計算（目視確認）に役立てる。

\subsection{To-Do List (タスク管理)}
